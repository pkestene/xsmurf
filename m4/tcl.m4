...
AC_DEFUN([SC_PATH_TCLCONFIG], [
	...
	AC_CACHE_VAL(ac_cv_c_tclconfig,[
	    ...
	    # add tcl8.3 tcl8.4 and tcl8.5 (P. Kestener)
	    if test x"${ac_cv_c_tclconfig}" = x ; then
		for i in `ls -d ${libdir} 2>/dev/null` \
			...
			`ls -d /usr/lib64/tcl8.5 2>/dev/null` \
			`ls -d /usr/lib64/tcl8.4 2>/dev/null` \
			`ls -d /usr/lib64/tcl8.3 2>/dev/null` \
...
AC_DEFUN([SC_PATH_TKCONFIG], [
	...
	AC_CACHE_VAL(ac_cv_c_tkconfig,[
		...
		for i in `ls -d ${libdir} 2>/dev/null` \
			...
			`ls -d /usr/lib64 2>/dev/null` \
			`ls -d /usr/lib 2>/dev/null` \
                        `ls -d /usr/lib64/tk8.5 2>/dev/null` \
                        `ls -d /usr/lib64/tk8.4 2>/dev/null` \
                        `ls -d /usr/lib64/tk8.3 2>/dev/null` \
			...
			`ls -d /usr/local/lib/tk8.4 2>/dev/null` \
			`ls -d /usr/local/lib/tk8.3 2>/dev/null` \
			; do
...
AC_DEFUN([SC_LOAD_TCLCONFIG], [
    ...
    #
    # PK: the following is taken from TkGate sources
    # (http://sourceforge.net/projects/tkgate)
    # normally TCL_INCLUDE_SPEC should be set by tclConfig.sh ...
    #   	
    # JPH: If TCL_INCLUDE_SPEC is not set, go try to find it elsewhere...
    #	
    if test "X${TCL_INCLUDE_SPEC}" = "X"; then
	    dirs="/usr/include /usr/unsupported/include /usr/local/include /usr/local/include/tcl8.5 /usr/local/include/tcl8.4 /usr/local/include/tcl8.3 /usr/local/include/tcl /usr/X386/include /usr/X11R6/include /usr/X11R5/include /usr/include/X11R5 /usr/include/X11R4 /usr/openwin/include /usr/X11/include /usr/sww/include /usr/sw/include"
		for i in $dirs; do
		    if test -f "$i/tcl.h" ; then
			TCL_INCLUDE_SPEC="-I$i"
			break
		    fi
		done
    fi
