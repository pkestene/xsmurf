 ...
 *
 *   $Id: cv1d_misc.c,v 1.12 1999/05/05 20:42:25 decoster Exp $
 */
 ...
 */
int
cv1d_convolution_method_ (int signal_size,
			  int filter_size,
 ...
 */
real *
cv1d_pure_periodic_extend_ (real *source_data,
			    int  begin,
			    int  end,
			    int  new_size)
{
 ...
 */
complex *
cv1d_pure_cplx_periodic_extend_ (complex *source_data,
				 int     begin,
				 int     end,
				 int     new_size)
{
 ...
 */
real *
cv1d_mirror_transform_ (real *source_data,
			int  size,
			int  cut)
{
  real *result;
 ...
 */
complex *
cv1d_cplx_mirror_transform_ (complex *source_data,
			     int     size,
			     int     cut)
{
  complex *result;
 ...
 */
real *
cv1d_padding_transform_ (real *source_data,
			 int  size,
			 int  cut)
{
  real *result;
...
real *
cv1d_0_padding_transform_ (real *source_data,
			   int  size,
			   int  cut)
{
  real *result;
 ...
 */
complex *
cv1d_cplx_padding_transform_ (complex *source_data,
			      int     size,
			      int     cut)
{
  complex *result;
 ...
 */
complex *
cv1d_cplx_0_padding_transform_ (complex *source_data,
				int     size,
				int     cut)
{
  complex *result;
...
void
cv1d_get_part_r_ (real *signal_part,
		  int  part_size,
		  real *signal_data,
		  int  signal_size,
		  int  part_begin_in_signal,
...
void
cv1d_get_part_c_ (complex *signal_part,
		  int     part_size,
		  complex *signal_data,
		  int     signal_size,
		  int     part_begin_in_signal,
 ...
 */
void
cv1d_set_f_l_exact_ (int *first_exact_ptr,
	       int *last_exact_ptr)
{
  int i_begin;
  int i_end;
  ...
  case ANALYTICAL:
    if (flt.scale == CV1D_NO_SCALE) {
      i_begin = (int) floor (flt.d_begin);
      i_end   = (int) ceil  (flt.d_end);
    } else {
      i_begin = (int) floor (flt.d_begin*flt.scale);
      i_end   = (int) ceil  (flt.d_end*flt.scale);
    }
    *first_exact_ptr = (int) i_end;
    *last_exact_ptr  = sig_n + (int) i_begin - 1;
    break;
  case NUMERICAL:
    *first_exact_ptr = (int) flt.d_end;
    *last_exact_ptr  = sig_n + (int) flt.d_begin - 1;
    break;
  }
}

...
int fftw_time_max_size = 65536;

real
my_fftw_get_time (int size)
{
  int i = 0;
  ...
  if (i == ARR_SIZE) {
    ...
    k = t*log(2)/(n*s*log(s));
    good_size = cv1d_next_good_fft_size(size);
...
cv1d_get_good_part_size (int size)
{
  ...
  p_size = cv1d_next_good_fft_size(size + 1);
  fft_t = my_fftw_get_time(p_size);
  t = fft_t / (p_size - size);
  t_min = t;
  ze_size = p_size;

  while (t < 2*t_min && size*100 > p_size) {
    p_size = cv1d_next_good_fft_size(p_size + 1);
    fft_t = my_fftw_get_time(p_size);
    t = fft_t / (p_size - size);
    if (t < t_min) {
      t_min = t;
