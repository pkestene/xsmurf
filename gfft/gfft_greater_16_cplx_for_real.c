...
/*
 *
 * $Id: gfft_greater_16_cplx_for_real.c,v 1.1 1999/05/06 14:09:55 decoster Exp $
 *
*/
...
static void
gfft_16pts_cplx_specialise(complex *in);

void gfft_greater_16_cplx_for_real(complex *in, int n, int log2n)
{   
    register complex *part = in;
    
    register int fft_size;
    register int current_fft_size;
    register int fft_size_over_8;
    ...
    register real cos_incr,sin_incr;
    ...
    cos_incr = cos(2*M_PI/fft_size);
    sin_incr = sin(2*M_PI/fft_size);
	...
	butterfly_size = current_fft_size >> 1;
	fft_size_over_8 = current_fft_size >> 3;
	    
	cosinus =  (real)1.0;
	sinus   =  (real)0.0;

	for( k = 0 ; k < fft_size ; k += current_fft_size) 
	{
	    ...
	    /* */
	    top_butterfly    += fft_size_over_8;
	    bottom_butterfly += fft_size_over_8;
	...
	cos_tmp   = cosinus;
	sin_tmp = sinus; 
	cosinus = cos_tmp * cos_incr - sin_tmp  * sin_incr;
	sinus   = cos_tmp * sin_incr + cos_incr * sin_tmp;
	
	for(j = 1 ; j < (current_fft_size >> 3) ; j++) 
	{
	    ...
	    for( k = j ; k < fft_size ; k += current_fft_size) 
	    {
		...
		top_butterfly_1 += current_fft_size;
		bottom_butterfly_1 += current_fft_size;
		...
		top_butterfly_2 += current_fft_size;
		bottom_butterfly_2 += current_fft_size;
		...
		top_butterfly_3 += current_fft_size;
		bottom_butterfly_3 += current_fft_size;
		...
		top_butterfly_4 += current_fft_size;
		bottom_butterfly_4 += current_fft_size;
	...
	cos_incr   = cos_tmp * cos_tmp - sin_tmp * sin_tmp;
	sin_incr   = cos_tmp * sin_tmp + cos_tmp * sin_tmp;
    ...
    {
	gfft_16pts_cplx_specialise(part);
	part += 16;
    }
}
...
gfft_16pts_cplx_specialise(complex *in)
{
    ...
    register real diff_10_real,diff_10_imag;
    ...
    diff_7_real = c1_real - c5_real;
    diff_7_imag = c1_imag - c5_imag;
    cc5_real =  COS_PI_4 * (diff_7_real + diff_7_imag);
    cc5_imag =  COS_PI_4 * (diff_7_imag - diff_7_real);
    ...
    diff_9_real = c3_real - c7_real;
    diff_9_imag = c3_imag - c7_imag;
    cc7_real =  - COS_PI_4  * ( diff_9_real - diff_9_imag);
    cc7_imag =  - COS_PI_4 * ( diff_9_imag + diff_9_real);
    ...
    diff_4_real = in[5].real - in[13].real;
    diff_4_imag = in[5].imag - in[13].imag;
    c13_real = ( - SIN_PI_8 * diff_4_real + COS_PI_8 * diff_4_imag);
    c13_imag = ( - SIN_PI_8 * diff_4_imag - COS_PI_8 * diff_4_real);

    in[5].real = ccc4_real - ccc5_real;
    in[5].imag = ccc4_imag - ccc5_imag;

    diff_5_real = in[6].real - in[14].real;
    diff_5_imag = in[6].imag - in[14].imag;
    c14_real =  - COS_PI_4 * (diff_5_real - diff_5_imag);
    c14_imag =  - COS_PI_4 * (diff_5_imag + diff_5_real);

    in[6].real = ccc6_real + ccc7_real;
    in[6].imag = ccc6_imag + ccc7_imag;

    diff_6_real  = in[7].real - in[15].real;
    diff_6_imag  = in[7].imag - in[15].imag;
    c15_real = ( - COS_PI_8 * diff_6_real + SIN_PI_8 * diff_6_imag);
    c15_imag = ( - COS_PI_8 * diff_6_imag - SIN_PI_8 * diff_6_real);
    ...
    diff_8_real = c9_real - c13_real;
    diff_8_imag = c9_imag - c13_imag;
    cc13_real =  COS_PI_4 * (diff_8_real + diff_8_imag);
    cc13_imag =  COS_PI_4 * (diff_8_imag - diff_8_real);

    cc11_real = c11_real + c15_real;
    cc11_imag = c11_imag + c15_imag;
    
    diff_10_real = c11_real - c15_real;
    diff_10_imag = c11_imag - c15_imag;
    cc15_real =  - COS_PI_4 * ( diff_10_real - diff_10_imag);
    cc15_imag =  - COS_PI_4 * ( diff_10_imag + diff_10_real);
