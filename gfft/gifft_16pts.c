...
/*
 *
 * $Id: gifft_16pts.c,v 1.1 1999/05/06 14:09:55 decoster Exp $
 *
*/
# include <gfft.h>
...
gifft_16pts_cplx(complex *in,complex *out)
{
    ...
    register real scale = (real)0.0625;
    ...
    out[0].real = (ccc0_real + ccc1_real) * scale ;
    out[0].imag = (ccc0_imag + ccc1_imag) * scale ;
    out[8].real = (ccc0_real - ccc1_real) * scale ;
    out[8].imag = (ccc0_imag - ccc1_imag) * scale ;
    
    out[4].real = (ccc2_real + ccc3_real) * scale ;
    out[4].imag = (ccc2_imag + ccc3_imag) * scale ;
    out[12].real = (ccc2_real - ccc3_real) * scale ;
    out[12].imag = (ccc2_imag - ccc3_imag) * scale ;

    out[2].real = (ccc4_real + ccc5_real) * scale ;
    out[2].imag = (ccc4_imag + ccc5_imag) * scale ;
    out[10].real = (ccc4_real - ccc5_real) * scale ;
    out[10].imag = (ccc4_imag - ccc5_imag) * scale ;

    out[6].real = (ccc6_real + ccc7_real) * scale ;
    out[6].imag = (ccc6_imag + ccc7_imag) * scale ;
    out[14].real = (ccc6_real - ccc7_real) * scale ;
    out[14].imag = (ccc6_imag - ccc7_imag) * scale ;
    ...
    out[1].real = (ccc8_real + ccc9_real) * scale ;
    out[1].imag = (ccc8_imag + ccc9_imag) * scale ;
    out[9].real = (ccc8_real - ccc9_real) * scale ;
    out[9].imag = (ccc8_imag - ccc9_imag) * scale ;

    out[5].real = (ccc10_real + ccc11_real) * scale ;
    out[5].imag = (ccc10_imag + ccc11_imag) * scale ;
    out[13].real = (ccc10_real - ccc11_real) * scale ;
    out[13].imag = (ccc10_imag - ccc11_imag) * scale ;

    out[3].real = (ccc12_real + ccc13_real) * scale ;
    out[3].imag = (ccc12_imag + ccc13_imag) * scale ;
    out[11].real = (ccc12_real - ccc13_real) * scale ;
    out[11].imag = (ccc12_imag - ccc13_imag) * scale ;

    out[7].real = (ccc14_real + ccc15_real) * scale ;
    out[7].imag = (ccc14_imag + ccc15_imag) * scale ;
    out[15].real = (ccc14_real - ccc15_real) * scale ;
    out[15].imag = (ccc14_imag - ccc15_imag) * scale ;
...
gifft_16pts_real(complex *in,real *out)
{
    ...
    register real cc8_real;
    register real cc9_real,cc9_imag;
    register real cc10_real;
    register real cc12_real;
    register real cc13_real,cc13_imag;
    register real cc14_real;

    register real ccc0_real;
    register real ccc1_real;
    register real ccc2_real;
    register real ccc3_real;
    register real ccc4_real;
    register real ccc5_real;
    register real ccc6_real;
    register real ccc7_real;
    register real ccc8_real;
    register real ccc9_real;
    register real ccc10_real;
    register real ccc11_real;
    register real ccc12_real;
    register real ccc13_real;
    register real ccc14_real;
    register real ccc15_real;

    c0_real = in[0].real + in[0].imag;
    c8_real = in[0].real - in[0].imag;
    
    diff_0_real = in[1].real - in[7].real;
    diff_0_imag = in[1].imag + in[7].imag;
    c1_real = in[1].real + in[7].real;
    c1_imag = in[1].imag - in[7].imag;
    c9_real = (COS_PI_8 * diff_0_real - SIN_PI_8 * diff_0_imag);
    c9_imag = (COS_PI_8 * diff_0_imag + SIN_PI_8 * diff_0_real);
    
    diff_1_real = in[2].real - in[6].real;
    diff_1_imag = in[2].imag + in[6].imag;
    c2_real = in[2].real + in[6].real;
    c2_imag = in[2].imag - in[6].imag;
    c10_real = COS_PI_4 * (diff_1_real - diff_1_imag);
    c10_imag = COS_PI_4 * (diff_1_imag + diff_1_real);
    
    diff_2_real = in[3].real - in[5].real;
    diff_2_imag = in[3].imag + in[5].imag;
    c3_real  = in[3].real + in[5].real;
    c3_imag  = in[3].imag -in[5].imag;
    c11_real = (SIN_PI_8 * diff_2_real - COS_PI_8 * diff_2_imag);
    c11_imag = (SIN_PI_8 * diff_2_imag + COS_PI_8 * diff_2_real);
    
    c4_real = in[4].real + in[4].real;
    c4_imag = in[4].imag - in[4].imag;
    c12_real = -in[4].imag - in[4].imag;
    c12_imag =  in[4].real - in[4].real;
    ...
    diff_6_real = c1_real - c3_real;
    diff_6_imag = c1_imag + c3_imag;
    cc1_real = c1_real + c3_real;
    cc1_imag = c1_imag - c3_imag;
    cc5_real =  COS_PI_4 * (diff_6_real - diff_6_imag);
    cc5_imag =  COS_PI_4 * (diff_6_imag + diff_6_real);
    
    diff_7_real = c9_real - c11_real;
    diff_7_imag = c9_imag + c11_imag;
    cc9_real = c9_real + c11_real;
    cc9_imag = c9_imag - c11_imag;
    cc13_real =  COS_PI_4 * (diff_7_real - diff_7_imag);
    cc13_imag =  COS_PI_4 * (diff_7_imag + diff_7_real);
    
    cc2_real = c2_real + c2_real;
    cc6_real = -(c2_imag + c2_imag);
    cc10_real = c10_real + c10_real;
    cc14_real = -(c10_imag + c10_imag);
    ...
    ccc12_real = cc12_real + cc14_real;
    ccc14_real = cc12_real - cc14_real;
    ccc1_real  = cc1_real + cc1_real;
    ccc3_real  = -(cc1_imag + cc1_imag);
    ccc5_real  = cc5_real + cc5_real;
    ccc7_real  = -(cc5_imag + cc5_imag);
    ccc9_real  = cc9_real + cc9_real;
    ccc11_real = -(cc9_imag + cc9_imag);
    ccc13_real = cc13_real + cc13_real;
    ccc15_real = -(cc13_imag + cc13_imag);

    out[0]  = (ccc0_real + ccc1_real) * scale;
    out[8]  = (ccc0_real - ccc1_real) * scale;
    out[4]  = (ccc2_real + ccc3_real) * scale;
    out[12] = (ccc2_real - ccc3_real) * scale;
    out[2]  = (ccc4_real + ccc5_real) * scale;
    out[10] = (ccc4_real - ccc5_real) * scale;
    out[6]  = (ccc6_real + ccc7_real) * scale;
    out[14] = (ccc6_real - ccc7_real) * scale;
    out[1]  = (ccc8_real + ccc9_real) * scale;
    out[9]  = (ccc8_real - ccc9_real) * scale;
    out[5]  = (ccc10_real + ccc11_real) * scale;
    out[13] = (ccc10_real - ccc11_real) * scale;
    out[3]  = (ccc12_real + ccc13_real) * scale;
    out[11] = (ccc12_real - ccc13_real) * scale;
    out[7]  = (ccc14_real + ccc15_real) * scale;
    out[15] = (ccc14_real - ccc15_real) * scale;
}
