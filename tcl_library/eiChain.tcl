...
#
#   -connected [float float] : chain only those maxima chains that are connected 
#                              and for which the segmented objects have a radius
#                              between radius_min and radius_max.
#
#   -connected_simple [float float] : Same as connected but without diameter, fil index, etc
#                                     
#                                     
#
#
...
proc chain {name aMin nO nV args} {
	...
	switch -exact -- [lindex $args 0] {
	    ...
	    -connected {
		set isConnected yes
		set radius_min [lindex $args 1]
		set radius_max [lindex $args 2]
		set args [lreplace $args 0 2]		
	    }
	    -connected_simple {
		set isConnected_simple yes
		set radius_min [lindex $args 1]
		set radius_max [lindex $args 2]
		set args [lreplace $args 0 2]		
	    }
	...
	for {set v 0} {$v < $nV} {incr v; incr memSid; incr fileSid} {
	    ...
	    if {$isConnected == "yes"} {
		hsearch_with_diam $zeEi
		ekeep_circle $zeEi $zeEi $radius_min $radius_max
	    }

	    if {$isConnected_simple == "yes"} {
		hsearch $zeEi
		ekeep_circle_simple $zeEi $zeEi $radius_min $radius_max
	    }
